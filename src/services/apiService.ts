...
// --- SEED DATA (Previously in data.ts) ---
...
const DB_KEY = 'fpt-app-data';
const LATENCY = 500; // ms in a realistic app this would be higher

const performAsync = <T>(operation: () => T): Promise<T> => {
...
const getDb = (): AppData => JSON.parse(localStorage.getItem(DB_KEY) || 'null');
const setDb = (data: AppData) => localStorage.setItem(DB_KEY, JSON.stringify(data));
...
export const apiService = {
    getInitialData: () => performAsync(() => getDb()),

    updateData: (updates: Partial<AppData>) => performAsync(() => {
        const db = getDb();
